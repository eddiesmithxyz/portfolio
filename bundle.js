...
(() => {
  ...
  var particleSize = wgslNumStr(0.9);
  var renderShaders = (
    /* wgsl */
    `
struct Uniforms {
...
`
  ...
  var WGPURenderer = class {
    ...
    async createBuffersAndPipeline(instanceCount) {
      ...
      await debugWait("create render shaders");
      ...
      await debugWait("create render buffers");
  ...
  var smoothingRadius = 0.9;
  var bound = vec3.create(150, 150, 25);
...
`
  ...
  var positionStiffness = wgslNumStr(-1);
...
`
  ...
  var WGPUComputer = class {
    ...
    maxParticleCount;
    spawnPeriod = 3;
    // duration (seconds) of initial period where particles spawn in
    particleDataBuffer0;
    ...
    constructor(device, particleCount2, initialInstanceData, renderInstanceBuffer) {
      this.device = device;
      this.maxParticleCount = particleCount2;
  ...
  var Scene = class {
    viewDistance = 84;
    camPos = vec3.create(0, 0, this.viewDistance);
    viewMatrix = mat4.lookAt(this.camPos, [0, 0, 0], [0, 1, 0]);
  ...
  async function main() {
    ...
    await debugWait("create data");
    const particleData = scene.createInitialParticleData(particleCount);
    await renderer.createBuffersAndPipeline(particleCount);
    await debugWait("create computer");
