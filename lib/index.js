...
var WGPURenderer = class {
  ...
  createBuffersAndPipeline(params) {
  ...
  render(viewProjectionMatrix, camPos) {
    if (!this.initialised) {
...
const groupSideForce = ${wgslNumStr(groupSideForce)};
...
// src/ts/compute/shader/physics/sdf/primitives.ts
var primitiveSDFsSrc = (
  /* wgsl */
  `
...
// src/ts/compute/shader/physics/sdf/letters.ts
var letterSDFsSrc = (
  /* wgsl */
  `
${primitiveSDFsSrc}
...
// src/ts/compute/shader/physics/sdf/word.ts
var wordSDFSrc = (
  /* wgsl */
  `
${letterSDFsSrc}
...
fn sdfEddie(p: vec3<f32>) -> f32 {
    ...
    // E
    minDist = min(minDist, sdE(pos - vec3<f32>(-2.05, 0.0, 0.5), r) * scale);
...
// src/ts/compute/shader/physics/sdf/sdf.ts
var mass = wgslNumStr(0.05);
var positionStiffness = wgslNumStr(-1.8);
var velocityDamping = wgslNumStr(-1.3);
var gravityClamp = wgslNumStr(400);
var mouseDisturbRadius = wgslNumStr(3);
var mouseDisturbSharpness = wgslNumStr(20);
var sdfSrc = (
  /* wgsl */
  `
${wordSDFSrc}
...
var WGPUComputer = class {
  ...
  maxParticleCount;
  spawnPeriod = 3;
  // duration (seconds) of initial period where particles spawn in. not sure if working
  particleDataBuffer0;
  ...
  resultBuffer;
  // for debug
  constructor(device, particleCount, initialInstanceData, renderInstanceBuffer) {
    this.device = device;
  ...
  async run(deltaTime, scene2) {
    ...
    this.uniforms.get("time").value[0] = scene2.time;
    this.uniforms.get("deltaTime").value[0] = deltaTime;
    this.uniforms.get("mouseIntersection").value = scene2.mouseIntersection;
    this.uniforms.get("lastMouseIntersection").value = scene2.lastMouseIntersection;
    this.uniforms.get("animSpeed").value[0] = window.PAUSE_UPDATE ? 0 : 1;
    this.uniforms.get("particleCount").value[0] = this.particleCount;
    this.uniforms.get("camPos").value = scene2.camPos;
...
function render(renderer, computer) {
  ...
  scene.update(renderer.ctx.canvas, deltaTime);
...
var running = false;
async function runSim(canvas, params = void 0) {
  if (running) return;
  running = true;
  let finalParams = {
    ...defaultParams,
  ...
  if (finalParams.autoResize) {
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        const canvas2 = entry.target;
        canvas2.width = Math.max(1, Math.min(width, renderer.device.limits.maxTextureDimension2D));
        canvas2.height = Math.max(1, Math.min(height, renderer.device.limits.maxTextureDimension2D));
      }
    });
